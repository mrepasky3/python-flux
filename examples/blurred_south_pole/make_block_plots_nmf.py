...
if compression_type == "true_model":
    FF_dir = "true_{}_{}_{}".format(max_area_str, outer_radius_str)
...
if compression_type == 'true_model':
    FF_path = 'results/' + FF_dir + f'/FF_{max_area_str}_{outer_radius_str}.npz'
    if not os.path.exists(FF_path):
        print("PATH DOES NOT EXIST " + FF_path)
        assert False
    FF = scipy.sparse.load_npz(FF_path)
else:
    FF_path =  "results/"+ FF_dir + "/FF_{}_{}_{:.0e}_{}.bin".format(max_area_str, outer_radius_str, args.tol, compression_type)
    if not os.path.exists(FF_path):
        print("PATH DOES NOT EXIST " + FF_path)
        assert False
